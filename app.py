...
def load_and_train():
    ...
    cols_to_drop = ['id', 'Gender', 'Profession', 'Work Pressure', 'Job Satisfaction']
    ...
    ohe_city = enc_city.fit_transform(df[['City']])
    city_cols = [f"City_{c}" for c in enc_city.categories_[0]]
    ohe_city_df = pd.DataFrame(ohe_city.toarray(), columns=city_cols, index=df.index)

    df_final = pd.concat(
        [df.drop(columns=['Degree', 'Family History of Mental Illness', 'City']),
         ohe_degree_df, ohe_family_df, ohe_city_df],
        axis=1
    )
    ...
    meta = {
        ...
        "city_categories": list(enc_city.categories_[0]),
...
with st.expander("ℹ️ Tentang model ini"):
    st.write(
        ...
        pola makan, jenjang pendidikan, jam belajar/kerja, tekanan finansial, riwayat keluarga
        dengan gangguan mental, dan kota domisili.
        """
    )
...
with col1:
    ...
    cgpa = st.number_input("CGPA / IPK (skala 0-10)", min_value=0.0, max_value=10.0, value=7.5, step=0.01)
...
# Kota: filter opsi yang terlihat seperti nama kota asli (bukan data kotor)
city_options = sorted(meta["city_categories"])
city = st.selectbox("Kota Domisili", city_options, index=0,
                     help="Beberapa entri di dataset asli tidak valid sebagai nama kota (data kotor bawaan sumber data) — silakan pilih yang paling sesuai atau terdekat.")
...
if st.button("🔍 Cek Hasil Skrining", use_container_width=True):
    ...
    city_vec = objs["enc_city"].transform([[city]]).toarray()
    city_df = pd.DataFrame(city_vec, columns=[f"City_{c}" for c in objs["enc_city"].categories_[0]])

    full_row = pd.concat([base_df, degree_df, family_df, city_df], axis=1)
